...
export default function VendorOnboarding() {
    ...
    useEffect(() => {
        const init = async () => {
            ...
            const { data: userData } = await supabase.from('users').select('approval_status').eq('id', user.id).single();
            if (userData) setServerStatus(userData.approval_status);
            ...
            <div className="w-full max-w-4xl mx-auto flex flex-col items-center">
                ...
                <div className="text-center mb-8">
                    <h1 className="text-2xl sm:text-4xl font-black tracking-tight mb-2">
                        {isEditMode ? 'Complete Store Profile' : 'Set Up Your Online Store'}
                    </h1>
                ...
                <div className="w-full bg-card rounded-[1.5rem] sm:rounded-[2.5rem] border border-border shadow-sm overflow-hidden relative min-h-[500px]">
                    {/* Progress Steps */}
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        {step === 1 && (
                            <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="p-5 sm:p-12">
                        ...
                        {/* Step 2: KYC & Bank */}
                        {step === 2 && (
                            <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="p-5 sm:p-12">
                                <div className="flex items-center gap-3 mb-8">
                        ...
                        {step === 4 && (
                            <motion.div key="step4" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="p-8 md:p-16 flex flex-col items-center text-center">
                                {serverStatus === 'rejected' ? (
                                    <>
                                        <div className="bg-red-500/10 text-red-500 rounded-full p-6 mb-6 animate-pulse">
                                            <X size={56} strokeWidth={2.5} />
                                        </div>
                                        <h2 className="text-3xl font-black mb-3 text-red-600">Application Declined</h2>
                                        <p className="text-muted-foreground text-sm mb-6 max-w-md">
                                            Administrators have reviewed and declined your application for the following reason(s). Please fix these and re-submit.
                                        </p>
                                        <div className="bg-red-500/5 border-2 border-red-500/20 w-full max-w-md rounded-2xl p-6 text-left mb-8 shadow-sm">
                                            <p className="text-[10px] font-black text-red-500 uppercase tracking-widest mb-2 flex items-center gap-2">
                                                🚩 Reason for Rejection
                                            </p>
                                            <p className="text-sm font-bold text-foreground">
                                                {rejectionReason || "No specific reason provided. Please contact support or check your documents."}
                                            </p>
                                        </div>
                                        <div className="flex gap-4">
                                            <Button onClick={() => setStep(1)} variant="outline" className="rounded-xl font-bold px-8 h-12">
                                                Fix & Re-submit
                                            </Button>
                                            <Button onClick={() => navigate('/vendor')} variant="ghost" className="rounded-xl font-bold px-8 h-12 text-muted-foreground">
                                                Go to Dashboard
                                            </Button>
                                        </div>
                                    </>
