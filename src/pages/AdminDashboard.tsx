...
// Feature tabs
import {
    AdminAnalyticsTab, AdminVendorsTab, AdminProductsTab,
    AdminEmiTab, AdminPaymentsTab, AdminPlatformTab
} from '../features/admin';
...
export default function AdminDashboard() {
    ...
    const handleApprove = async (userId: string) => {
        ...
        else { toast.success('Vendor approved! They can now access their dashboard.'); fetchVendors(); }
    };

    const handleDecline = async (userId: string) => {
        const { error } = await supabase.from('users').update({ approval_status: 'rejected' }).eq('id', userId);
        if (error) { toast.error('Failed to decline: ' + error.message); }
        else { toast.success('Vendor application declined.'); fetchVendors(); }
    };
